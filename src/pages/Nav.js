...
export function Nav() {
    ...
    return (
        <nav className='hidden md:flex justify-center mt-4 shadow-[0_10px_10px_-15px_rgba(0,0,0,0.3)] text-yellow-950 border-y border-y-yellow-950 w-[95%] mx-auto'>
        {[{text: "Trang chủ", link: "/"}, 
          ...
          <div key={i.text} className='relative text-center'  onMouseEnter={() => {i.text === "Sản phẩm" && setHover("")}} onMouseLeave={() => {i.text === "Sản phẩm" && setHover("hidden")}}>
            <Link reloadDocument to={i.link}><div className='cursor-pointer text-[17px] font-semibold hover:scale-105 transition mx-4 py-1'>{i.text}</div></Link>
            {i.text === "Sản phẩm" && <div className={'absolute border border-gray-200 text-sm w-64 left-0 top-10 bg-white flex flex-col justify-center items-center pb-2 shadow-[0_30px_30px_-15px_rgba(0,0,0,0.3)] ' + isHover}>
