...
export function Nav() {
    ...
    return (
        <nav className='hidden md:flex justify-center mt-4 shadow-[0_10px_10px_-15px_rgba(0,0,0,0.3)] text-yellow-950'>
        {[{text: "Trang chủ", link: "/"}, 
          ...
          <div key={i.text} className='relative text-center'  onMouseEnter={() => {i.text === "Sản phẩm" && setHover("")}} onMouseLeave={() => {i.text === "Sản phẩm" && setHover("hidden")}}>
            <Link reloadDocument to={i.link}><div className='cursor-pointer text-base font-semibold hover:scale-105 transition mx-4 pb-4'>{i.text}</div></Link>
